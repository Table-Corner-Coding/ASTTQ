...
function update_from_transient() {
	...
	foreach($posts_to_update as $current_post){
		...
		foreach($the_post_meta as $key=>$value){
			...
			$retVal .= '<tr><td>'.$key.' => '.$value[0].'</td><td>('.$the_post_id.')'.$the_post_obj->post_title.'</td><td>'.strftime('%d/%m/%y - %H:%M').'</td></tr>';
		}
	...
	$termine = 0;
			...
			if(is_user_logged_in() && current_user_can('manage_options')){
				...
				foreach($events as $event){
					...
					$tabs .= '<h3>'.tribe_get_venue($event->ID).' - '.tribe_get_start_date ( $event->ID, false, 'j F' ).'</h3>';
					...
					foreach($classes as $classe){
						...
						$tabs .= '<div class="bonus"><strong>Bonus de points</strong><input type="number" class="bonus_points" value="'.$bonus.'" /></div>';
							$tabs .= '<table data-event-id="'.$event->ID.'" data-term-id="'.$classe->term_id.'" class="editable_table comp_table"><thead><tr><th><span title="Trier le tableau" class="dashicons dashicons-randomize"></span><span class="dashicons dashicons-media-document" title="imprimer"></span></th><th>'.__('Véhicule','asttq').'</th><th>'.__('Conducteur','asttq').'</th><th>'.__('Distances','asttq').'</th><th>'.__('Membre','asttq').'</th><th>Actions</th></tr></thead><tbody>';
						...
						if(!empty($compArr[$objID])){
							...
							foreach($tireurs as $tireur){
								...
								$tabs .=  '<tr class="tireur_line" data-Tireur-ID="'.$tireur_id.'"><td class="pos"><input class="num" value="" type="number" /></td><td data-content="'.$vehicule.'" class="vehicule">';
