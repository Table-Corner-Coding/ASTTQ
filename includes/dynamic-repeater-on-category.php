<?php 
	...
	class my_dynamic_acf_extension {
		...
		public function load_features_from_term() {
			...
			// $current_term = get_field('classe', $post_id);
			$current_term = $term_id;
			/*
			if ($current_term == $term_id) {
				//echo 'get_current_values'; exit;
				if (have_rows('competiteur', $post_id)) {
					//echo 'have_rows'; exit;
					while(have_rows('competiteur', $post_id)) {
						the_row();
						$values[] = array(
			...
			*/
			...
			if (have_rows('competiteur', $taxonomy.'_'.$term_id)) {
				while (have_rows('competiteur', $taxonomy.'_'.$term_id)) {
					the_row();
					$values[] = array(
						// key value pairs for fields to be updated
						// by callback function
						'field_598a3139a9622' => get_sub_field('tireur')
					);
				} // end while have rows
			} // end if have rows
		...
		public function enqueue_script() {
			...
			*/
			...
			if (!$post ||
			    !isset($post->ID) || 
			    get_post_type($post->ID) != 'tribe_events') {
				return;
			}
